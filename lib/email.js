...
export async function sendInvite(targets, inviter_name, room_name) {
  for (let i = 0; i < targets.length; i++) {
    await transporter.sendMail({
      ...
      html: `${inviter_name} has invited you to a meeting! You can join it here: ${process.env.NEXT_PUBLIC_IS_DEV === "TRUE" ? "http://localhost:3000" : "https://" + process.env.NEXT_PUBLIC_DOMAIN}/call?room_name=${encodeURIComponent(room_name)}.`,
    });
  }
}
